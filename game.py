...
class Game:
    ...
    def setup(self):
        ...
        clefs = Item("clefs", "Un trousseau de vieilles clés en fer forgé, lourdes et usées, dont certaines ne semblent ouvrir aucune porte visible", 0.2)
        manuscrit = Item("manuscrit", "Un manuscrit jauni, écrit à la main, relatant des événements troublants survenus autrefois dans le château", 0.35)
        coffret_souvenirs = Item("coffret_souvenirs", "Un petit coffret en bois sombre contenant quelques objets personnels, soigneusement conservés", 0.8)
        dague = Item("dague", "Une dague ancienne à la lame ternie, ornée de motifs gravés presque effacés", 0.6)
        bougies = Item("bougies", "Des bougies en cire épaisse, partiellement consumées, dégageant une légère odeur de cire froide", 0.15)
        allumettes = Item("allumettes", "Une petite boîte d’allumettes anciennes", 0.05)
        pistolet = Item("pistolet", "Un pistolet ancien, rangé avec soin, dont le mécanisme semble encore fonctionnel", 1)
        miroir = Item("miroir", "Un miroir au cadre doré craquelé, dont le reflet paraît parfois légèrement déformé", 4)
        portrait_familial = Item("portrait_familial", "Un grand portrait représentant une famille austère, dont certains regards semblent suivre l’observateur", 1)
        horloge = Item("horloge", "Une horloge murale imposante, dont le tic-tac résonne étrangement dans les couloirs vides", 12)
